...
#ifndef _MATERIALLITCLUSTERPROCESSRESULTS_H_
#define _MATERIALLITCLUSTERPROCESSRESULTS_H_
...
class MaterialLitClusterProcessResults : public Material
{
...
protected:
	...
	* @return nothing */
	MaterialLitClusterProcessResults(string &&name) : Material(move(name), move(string("MaterialLitClusterProcessResults")))
...
public:
	...
	* @return true if the shader was loaded successfully, and false otherwise */
	bool loadShader()
	{
		...
		void* shaderFirstPartCode  = InputOutput::readFile("../data/vulkanshaders/litclusterprocessresultsfirstpart.comp",  &sizeFirstPart);
		void* shaderSecondPartCode = InputOutput::readFile("../data/vulkanshaders/litclusterprocessresultssecondpart.comp", &sizeSecondPart);
		...
		m_shaderResourceName = "litclusterprocessresults";
	...
	* @return nothing */
	void exposeResources()
	{
		...
		exposeStructField(ResourceInternalType::RIT_UNSIGNED_INT, (void*)(&m_maximumThreadIndex),        move(string("myMaterialData")), move(string("maximumThreadIndex")));
		...
		assignShaderStorageBuffer(move(string("litClusterCounterBuffer")),                move(string("litClusterCounterBuffer")),                VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		assignShaderStorageBuffer(move(string("litVisibleClusterBuffer")),                move(string("litVisibleClusterBuffer")),                VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		assignShaderStorageBuffer(move(string("litToRasterVisibleClusterCounterBuffer")), move(string("litToRasterVisibleClusterCounterBuffer")), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		assignShaderStorageBuffer(move(string("litToRasterVisibleClusterBuffer")),        move(string("litToRasterVisibleClusterBuffer")),        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		...
		assignShaderStorageBuffer(move(string("alreadyRasterizedClusterBuffer")),         move(string("alreadyRasterizedClusterBuffer")),         VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
	...
	* @return nothing */
	...
	SET(uint, m_maximumThreadIndex, MaximumThreadIndex)
...
protected:
	...
	uint    m_maximumThreadIndex;                //!< Maximum thread index for the elements being processed
	vec4    m_sceneExtentAndVoxelSize;           //!< Extent of the scene in the xyz coordinates, voxelization texture size in the w coordinate
	vec4    m_sceneMin;                          //!< Minimum value of the scene's aabb in xyz coordinates
...
#endif _MATERIALLITCLUSTERPROCESSRESULTS_H_
